...
class SkunkSubmissionRepository[F[_]: Concurrent](session: Session[F]) extends SubmissionRepository[F] with Helpers:
  ...
  override def findById(id: SubmissionId): F[Option[Submission]] =
    repoHandler(session.prepare(SubmissionQueries.findById).flatMap(ps => ps.option(id)))

  override def create(submission: Submission): F[Submission] =
    repoHandler(session.prepare(SubmissionQueries.create).flatMap(ps => ps.unique(submission)))

  override def update(submission: Submission): F[Option[Submission]] =
    repoHandler(session.prepare(SubmissionQueries.update).flatMap(ps => ps.option(submission)))

  override def delete(id: SubmissionId): F[Boolean] =
    repoHandler(session.prepare(SubmissionQueries.delete).flatMap(ps => ps.execute(id).flatMap(wasDeleted)))
