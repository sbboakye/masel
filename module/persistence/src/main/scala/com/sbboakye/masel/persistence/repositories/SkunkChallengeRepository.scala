...
class SkunkChallengeRepository[F[_]: Concurrent](pool: Resource[F, Session[F]]) extends ChallengeRepository[F]:
  override def findAll(limit: Int, offset: Int): Stream[F, Either[AppError, Challenge]] =
    Stream
      .resource(pool)
      .flatMap { session =>
        Stream
  ...
  override def findById(id: ChallengeId): EitherT[F, AppError, Challenge] =
    EitherT(
      pool.use(session => session.prepare(ChallengeQueries.findById).flatMap(ps => ps.option(id))).flatMap {
